...
```bash
$ oc get pods <pod-name> -o=jsonpath='{.metadata.name}{"Â¥t"}{.metadata.annotations}{"Â¥n"}'
...
```bash
$ oc get scc --no-headers | awk '{print "oc get scc "$1" -o jsonpath=@{.metadata.name}{.groups}@; echo Â¥n"}' | sed 's/@/"/g' | sh
