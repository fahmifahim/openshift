...
$ minishift addon apply registry-route
 ...
 -- Adding the secret to the registry podâ€™s service accounts (including the default service account)..
 ...
 -- Updating the scheme used for the registryâ€™s liveness probe from HTTP to HTTPS.
 -- Updating the scheme used for the registryâ€™s readiness probe from HTTP to HTTPS.
...
ã€?1ã€‘Setting up the NFS server:
...
ã€?2ã€‘Create Persistent Volume (PV)
...
ã€?7ã€‘Verifying current configuration
