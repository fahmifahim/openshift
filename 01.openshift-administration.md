...
6. Extract the current `localusers` secret to htpasswd file at /tmp/htpasswd-mgr. Add `manager` user with password `manager`. Change the password for `user2` to `pass222`. Update the secret and make sure you can login with manager user.  
...
#Check whether the secret is consumed safely by oauth pod
...
6. Extract the current `localusers` secret to htpasswd file at /tmp/htpasswd-mgr. Add `manager` user with password `manager`. Change the password for `user2` to `pass222`. Update the secret and make sure you can login with manager user.  
```bash
oc extract secret/localusers -n openshift-config --to /tmp/htpasswd-mgr --confirm
...
oc extract secret/localusers -n openshift-config --to /tmp/htpasswd-del --confirm

htpasswd -D /tmp/htpasswd-del user2

oc set data secret/localusers --from-file htpasswd=/tmp/htpasswd-del -n openshift-config
