...
1. Create secured passthrough route.  
Generate CSR for php-https.apps.ocp4.example.com.  (You can remove the -subj option, but dont forget to specify the detail entries on the prompt):  
- input key = `training.key` (already prepared)  
- define the output key as `training.csr`  
```bash  
#Generate CSR
openssl req -new -key training.key -out training.csr
#-> On the prompt, dont forget to mention the Common Name for php-https.apps.ocp4.example.com

#Generate the signed certificate
openssl x509 -req -in training.csr ¥
    -CA training-CA.pem -CAkey training-CA.key -CAcreateserial ¥
    -passin file:passphrase.txt ¥
    -out training.crt -day 3650 -sha256 -extfile training.ext

#Create TLS secret
oc create secret tls php-secret --cert training.crt --key training.key
```  

Deploy application with the below yaml  
```yaml
TBD
```  
