...
```bash
# `podman stop` to stop the official-nginx container.
...
6. Log in to the container using the exec subcommand to introduce a final change. Replace the contents of the file /usr/share/nginx/html/index.html file with DO180 Page.  
...
# Update the index.html file located at /usr/share/nginx/html. The file should read HELLO WORLD Page.
        root@02dbc348c7dc:/ echo 'HELLO WORLD Page' > /usr/share/nginx/html/index.html
...
8. Remove the development image test/mynginx:v1.0-SNAPSHOT from local image storage.
...
# Use the sudo podman rmi command to remove the test/mynginx:v1.0-SNAPSHOT image.
